...
import {
  Sparkles, Upload, FileText, Image as ImageIcon, X,
  ChevronDown, Loader2, Bot
} from 'lucide-react'
import { cn, TINGKAT_SEKOLAH, MATA_PELAJARAN_BY_TINGKAT, BENTUK_SOAL_LABELS, AI_AGENT_LABELS } from '@/lib/utils'
import type {
...
export default function DashboardPage() {
  ...
  const [jumlahSoal, setJumlahSoal] = useState<Record<BentukSoal, number>>({
    ...
    menjodohkan: 5, benar_salah: 5, teka_teki_silang: 1
  })
  ...
  function toggleBentuk(bentuk: BentukSoal) {
    setSelectedBentuk(prev => {
      ...
      if (next.has(bentuk)) {
        if (next.size === 1) return prev // minimal 1
        next.delete(bentuk)
      } else {
        next.add(bentuk)
      }
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      {/* Header */}
      <div>
        <h1 className="text-xl font-semibold text-slate-900">Buat Soal</h1>
        <p className="text-sm text-slate-400 mt-0.5">Isi parameter di bawah lalu masukkan materi untuk menghasilkan soal</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-5">
            <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Jenjang Sekolah</label>
                  ...
                  <div key={bentuk} className="space-y-1.5">
                    ...
                    {isSelected && (
                      <div className="flex items-center gap-1.5 px-1">
                        <span className="text-xs text-slate-400 shrink-0">Jumlah:</span>
                        <input
                          type="number"
                          min={1}
                          max={50}
